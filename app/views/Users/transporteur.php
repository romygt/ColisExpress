...
<link href="<?php echo URLROOT ?>/public/css2/styles.css" type="text/css" rel="stylesheet">
	...
	<div class="container">
		...
		<center><h1>Transporteurs</h1></center>
		...
		<table id="itemsTable" class="table"
			...
			<thead>
				<tr>
					...
					<th data-field="Bannir" data-align="" data-sortable="true" data-filter-type="input">Banir</th>
			...
			<tr>
				...
				<?php if(( $user->transporteur=='yes')&&($user->transporteurcertifie=='no')):?>
				<td><?php echo 'transporteur'?></td>
				<?php endif;?>
				<?php if(( $user->transporteur=='yes')&&($user->transporteurcertifie=='yes')):?>
				<td><?php echo 'transporteur certifie'?></td>
				<?php endif ;?>
				<td><?php echo $user->etat?></td>

				<?php if(( $user->etat=='En attente')):?>	
				<td><center><button onclick="location.href='<?php echo URLROOT; ?>/Users/validerTransporteur?id=<?php echo $user->iduser?>'" style="  color:#fff background-color: #ffa500" >Valider</button></center></td>				
				<?php else:?>
					<td></td>				
				<?php endif ;?>
				
				<?php if(( $user->etat!='Bannit')):?>	
				<td><center><button onclick="location.href='<?php echo URLROOT; ?>/Users/banirTransporteur?id=<?php echo $user->iduser ?>&trans=yes'" style="  color:#fff background-color: #ffa500" >Bannir</button></center></td>
				<?php else:?>
					<td></td>				
				<?php endif ;?>
